...
struct daily_filename_calculator2 {
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm) {
        ...
        std::tie(basename, ext) = details::FileHelperChacha::split_by_extension(filename);
...
public:
    ...
    daily_filechacha_sink(filename_t base_filename,
                    int rotation_hour,
                    int rotation_minute,
                    uint8_t* chacha_key, uint8_t* chacha_nonce,
                    bool truncate = false,
        ...
        : base_filename_(std::move(base_filename)),
          ...
          file_helper_{chacha_key,chacha_nonce,event_handlers},
          truncate_(truncate),
          max_files_(max_files),
          filenames_q_() {
        if (rotation_hour < 0 || rotation_hour > 23 || rotation_minute < 0 ||
            rotation_minute > 59) {
            throw_spdlog_ex("daily_filechacha_sink: Invalid rotation time in ctor");
        }
...
private:
    ...
    details::FileHelperChacha file_helper_;
    bool truncate_;
    uint16_t max_files_;
